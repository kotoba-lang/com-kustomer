(ns kustomer.main
  "Kotodama WASM entrypoint for the Kustomer clean-room actor (L4) — Clojure port.
  ...
  facts (`kustomer.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kustomer")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"kustomer.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kustomer-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kustomer-compat/ready)
